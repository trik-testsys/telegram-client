...
from controller.States import ChoseSubmit, ChoseTask
...
class ChoseSubmitStateController(Controller):
    ...
    async def create_CHOOSE_SUBMIT_KEYBOARD(self, message):
        ...
        submits = await self.submit_repository.get_student_submits_by_task(state_info.chosen_student,
    ...
    async def handle(self, message: Message):
        ...
        file = await self.grading_service.get_submit(submit_id)
        if file != self.grading_service.ERROR:
            await message.bot.send_document(message.from_user.id, (f'submit_{submit_id}.qrs', file))
        else:
    ...
    async def prepare(self, message: Message):
        await message.answer(self.CHOOSE_SUBMIT, reply_markup=await self.create_CHOOSE_SUBMIT_KEYBOARD(message))
