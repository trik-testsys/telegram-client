...
class TaskMenuStateController(Controller):
    ...
    BACK = "◂ Назад"
    SENT = "Бот принял вашу работу и начал её анализировать. " \
           "Обработка задания может занять несколько минут. " \
           "Таблицу с результатами обработки можно посмотреть в меню 'Результаты запусков'" \
           "Если задания выполнено правильно, то напротив ID появится '+'," \
           "если задание еще в обработке -- '?', " \
